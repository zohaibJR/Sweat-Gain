...
function Records() {
  ...
  const years = [];
  for (let year = 2024; year <= now.getFullYear(); year += 1) years.push(year);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        apiUrl(`/api/attendance/monthly/records?email=${email}&month=${selectedMonth}&year=${selectedYear}`)
      );
  ...
  const fetchExerciseData = useCallback(async () => {
    ...
    try {
      const [histRes, muscleRes, weeklyRes] = await Promise.all([
        axios.get(apiUrl(`/api/exercises/history?email=${email}&days=${exRange}`)),
        axios.get(apiUrl(`/api/exercises/muscle-stats?email=${email}`)),
        axios.get(apiUrl(`/api/exercises/weekly?email=${email}`)),
      ]);

      setExHistory(histRes.data || []);
      setMuscleStats(muscleRes.data.stats || []);
      setMostTrained(muscleRes.data.mostTrained || null);
  ...
  const filtered = records.filter((record) => {
    ...
    const sourceText = record.source === 'manual' ? 'manual check-in' : 'auto placeholder';
    const searchMatch =
      searchDate.trim() === '' ||
      record.formattedDate.toLowerCase().includes(searchDate.toLowerCase()) ||
      record.dayName.toLowerCase().includes(searchDate.toLowerCase()) ||
      sourceText.includes(searchDate.toLowerCase());
  ...
  const rangeBtn = (active) => ({ padding: '7px 18px', borderRadius: 8, cursor: 'pointer', fontFamily: 'Rajdhani,sans-serif', fontSize: 13, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', transition: 'all .2s', border: active ? '1px solid #00d4ff' : '1px solid rgba(26,111,212,.25)', background: active ? 'rgba(0,212,255,.12)' : 'rgba(10,22,40,.8)', color: active ? '#00d4ff' : 'rgba(255,255,255,.4)' });
  const pill = (color) => ({ background: `${color}15`, border: `1px solid ${color}40`, borderRadius: 12, padding: '16px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, minWidth: 100 });
  const bigNum = (color) => ({ fontFamily: 'Bebas Neue,sans-serif', fontSize: 34, color, lineHeight: 1 });
    ...
    <div className="RecordsPage">
      <div className="RecordsInner">
        <div className="RecordsHeader">
          ...
          <p className="RecordsSubtitle">Track auto placeholders, manual check-ins, and every weight update by month.</p>
        </div>
        ...
        <div className="RecordsControls">
          ...
          <div className="ControlGroup">
            ...
            <div className="FilterBtns">
              {['All', 'Present', 'Absent'].map((status) => (
                <button key={status} className={`FilterBtn ${statusFilter === status ? `active-${status.toLowerCase()}` : ''}`} onClick={() => setStatusFilter(status)}>
                  {status}
                </button>
              ))}
            </div>
          ...
          <div className="ControlGroup SearchGroup">
            <label>Search</label>
            <input type="text" placeholder="e.g. Monday, auto, Apr..." value={searchDate} onChange={(e) => setSearchDate(e.target.value)} />
          </div>
        </div>
        ...
        <div className="SundayLegend">
          ...
          <span>Every day starts as an auto absent placeholder. Entering weight changes that row to a manual Present check-in.</span>
        </div>
            ...
            <table className="RecordsTable">
              ...
              <tbody>
                {filtered.map((record, index) => (
                  <tr key={record.id || record.date} className={record.status === 'Present' ? 'row-present' : 'row-absent'}>
                    <td className="RowNum">{index + 1}</td>
                    <td className="RowDateDay">
                      <span className="RowDate">{record.formattedDate}</span>
                      <span className="RowDay">{record.dayName}</span>
                    </td>
                    <td className="RowWeight">{record.weight}<span>kg</span></td>
                    <td>
                      <span className={`StatusBadge badge-${record.status.toLowerCase()}`}>{record.status}</span>
                    </td>
                    <td>
                      <div className="RecordMeta">
                        <span className={`SourceBadge badge-source-${record.source || 'auto'}`}>
                          {record.source === 'manual' ? 'Manual Check-In' : 'Auto Placeholder'}
                        </span>
                        {record.canCheckIn && <span className="SourceNote">Open until {new Date(record.editableUntil).toLocaleString()}</span>}
                        {record.checkedInAt && <span className="SourceNote">Saved {new Date(record.checkedInAt).toLocaleString()}</span>}
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        {isPro ? (
                    ...
                    <>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 20 }}>
                        {weeklySummary.map((day, index) => (
                          <div key={index} style={{ background: day.exercises.length > 0 ? 'rgba(0,230,118,.08)' : 'rgba(255,255,255,.04)', border: day.exercises.length > 0 ? '1px solid rgba(0,230,118,.25)' : '1px solid rgba(255,255,255,.07)', borderRadius: 12, padding: '10px 14px', minWidth: 90 }}>
                            <div style={{ fontFamily: 'Bebas Neue,sans-serif', fontSize: 16, letterSpacing: 1, color: day.exercises.length > 0 ? '#00e676' : 'rgba(255,255,255,.25)' }}>{day.day}</div>
                            {day.exercises.length > 0 ? (
                              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
                                {day.exercises.map((exercise, exerciseIndex) => (
                                  <span key={exerciseIndex} style={{ background: 'rgba(0,212,255,.1)', border: '1px solid rgba(0,212,255,.25)', color: '#00d4ff', fontSize: 10, fontFamily: 'Rajdhani,sans-serif', fontWeight: 700, padding: '2px 7px', borderRadius: 20 }}>{exercise}</span>
                                ))}
                              </div>
                    ...
                    <>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 20 }}>
                        {muscleStats.map((stat, index) => (
                          <div key={index} style={{ display: 'flex', alignItems: 'center', gap: 8, background: stat.count > 0 ? 'rgba(255,152,0,.07)' : 'rgba(255,255,255,.03)', border: stat.count > 0 ? '1px solid rgba(255,152,0,.2)' : '1px solid rgba(255,255,255,.06)', borderRadius: 10, padding: '8px 14px' }}>
                            <span style={{ fontFamily: 'Rajdhani,sans-serif', fontSize: 14, fontWeight: 700, color: stat.count > 0 ? 'rgba(255,255,255,.8)' : 'rgba(255,255,255,.25)' }}>{stat.name}</span>
                            <span style={{ fontFamily: 'Bebas Neue,sans-serif', fontSize: 18, color: stat.count > 0 ? '#ff9800' : 'rgba(255,255,255,.15)', lineHeight: 1 }}>{stat.count}x</span>
                          </div>
                        ))}
                      </div>
                    </>
          ...
          <div style={{ marginTop: 56, background: 'linear-gradient(135deg,rgba(15,31,61,.8),rgba(10,22,40,.9))', border: '1px solid rgba(0,212,255,.2)', borderRadius: 20, padding: '40px 32px', textAlign: 'center', animation: 'fadeUp .7s .3s ease both' }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>PRO</div>
